%regions for triangles 1 and 2 with diagonal cut lines

function model = make_todorovic_benary_cross_1_2_diag(params)
...
for n=0:68
    block(442+n:442+n+1, 101:101+n+1)=0.5;
end
...
padded = model_pad_patch(img, params.img.size.h, params.img.size.w);
    ...
    %cutline for 1st triangle
    for n=0:34
        model.labeled_regions(511-n-1:511-n, 101+n:101+n+1)=2.5;
    end
    [i,j]=find(model.labeled_regions==2);
    n=size(i);
    for c=1:n
    ...
    n=size(i);
    for c=1:n
        model.labeled_regions(i(c),j(c))=2.0;
    end

    %cutline for 2nd triangle
    for n=0:48
    model.labeled_regions(512+n:512+n+1,382:382)=3.5;
    end
    [i,j]=find(model.labeled_regions==4);
    n=size(i);
    for c=1:n
        model.labeled_regions(i(c),j(c))=0.0;
    end
    [i,j]= find((model.labeled_regions)==3.5);
    n=size(i);
    for c=1:n
        model.labeled_regions(i(c),j(c))=4;
    end
