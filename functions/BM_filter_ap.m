...
function [params, filter_response, ap_filter_response] = BM_filter_ap(params)
...
ap_filter_response = cell(length(orientations), length(stdev_pixels));
...
for o = 1 : length(orientations)
    ...
    for f = 1 : length(stdev_pixels)
        ...
        if(params.filt.norm)
            ...
            ap_filter = ap_filter ./ norm(ap_filter(:)); % unit norm
        end
        ...
        if params.out.genImages
            ...
            img_txt = {['Initial Antiphase Filter: orientation ', num2str(orientations(o)), ...
                ', frequency (pixels) ', num2str(stdev_pixels(f))], ...
                ['min = ', num2str(min(min(filter))), ...
                ', max = ', num2str(max(max(filter)))]};
            filename = ['apfilter-', ...
                num2str(orientations(o)), '-', num2str(stdev_pixels(f)), '.png'];
            generate_image(ap_filter, img_txt, filename, [params.out.mainDir params.out.patternDir], 'jet')
        end
            
        
        % get filter response and save it
        filter_response{o, f} = ourconv(params.img.img, filter, 0.5);  % pad image with gray
        ap_filter_response{o, f} = ourconv(params.img.img, ap_filter, 0.5);  % pad image with gray
        ...
        if params.out.genImages
            ...
            temp_filter_response = ap_filter_response{o,f};
            img_txt = {['Initial Antiphase Filter Response: orientation ', num2str(orientations(o)), ...
                ', frequency (pixels) ', num2str(stdev_pixels(f))], ...
                ['min = ', num2str(min(min(temp_filter_response))), ...
                ', max = ', num2str(max(max(temp_filter_response)))]};
            filename = ['apfilterresponse-', num2str(orientations(o)), '-', ...
                num2str(stdev_pixels(f)), '.png'];
            generate_image(temp_filter_response, img_txt, filename, [params.out.mainDir params.out.patternDir], 'jet')
