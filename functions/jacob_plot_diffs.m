function [  ] = jacob_plot_diffs( diff_results, params, ordering )
...
for i=1:size(jacobdata, 2)
    ...
    title([params.out.figTitle, ' - results, ', jacobhumanlabels{i}]);
    hold all
    bar([Y_vals_t, jacobdatarelative(:, i)])
    legend(models{:}, jacobhumanlabels{i})
    set(gca, 'XTick', 1:numel(jacobstimlabels))
    set(gca,'XTickLabel',jacobstimlabels);
    if verLessThan('matlab', '8.4.0')
        rotateXLabels( gca(), 45 )
    ...
    %save the graph if params is specified
    if nargin > 1
        filename = ['ind_results_bar_graph_',num2str(i)];
        dir = params.out.mainDir;
