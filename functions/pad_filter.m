function [ padded_filter ] = pad_filter( filter, max_size )
...
if filter_y < max_size
    diff = max_size - filter_y;
    if diff == 1
        padded_filter = [zeros(max_size, 1), vert_padded_filter];
    else  %size difference greater than 1
        padding_size = max_size - filter_y;
        if mod(padding_size, 2) %number is odd
            right_pad = floor(padding_size/2);
            left_pad = right_pad + 1;
            padded_filter = [zeros(max_size, left_pad), vert_padded_filter, zeros(max_size, right_pad)];
        else  %number is even
