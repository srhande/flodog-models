...
stims = {
    {'make_bm_whites_thick(params)', 'WE-thick'};               % BM 1999
    {'make_bm_whites_thin_wide(params)', 'WE-thin-wide'};           % BM 1999
%     {'make_whites_andersen(params)', 'WE-Anderson'};
%     {'make_whites_howe(params)', 'WE-Howe'};
%     {'make_small_grating_induction(params)', 'Grating induction'};       % BM 1999 grating induction, many cycles
%     {'make_sbc_large(params)', 'SBC-Large'}                     % BM 1999
%     {'make_sbc_small(params)', 'SBC-Small'};
%     {'make_todorovic(params, ''in-small'')', 'Todorovic-in-small'};            % BM 1999, small todorovic cross
%     {'make_todorovic(params, ''in-large'')', 'Todorovic-in-large'};             % BM 1999, larger todorovic cross, still inside square
%     {'make_todorovic(params, ''equal'')', 'Todorovic-equal'};                % BM 1999, todorovic cross exactly covering square, sometimes reverse illusion is seen
%     {'make_todorovic(params, ''out'')', 'Todorovic-out'};                 % BM 1999, todorovic cross extending outside square
%  	{'make_check_5(params)', 'Checkerboard-0.16'};      % small checkerboard
%  	{'make_check_30(params)', 'Checkerboard-0.94'}; % larger checkerboard
%  	{'make_check_67(params)', 'Checkerboard-2.1'}; % largest checkerboard
%     {'make_mondrian_f(params)', 'Corrugated Mondrian'};
%     {'make_benary_cross(params)', 'Benary cross'};
%     {'make_todorovic_benary_cross_1_2_diag(params)', 'Todorovic Benary 1-2'};
%     {'make_todorovic_benary_cross_3_4_diag(params)', 'Todorovic Benary 3-4'};
...
plot_diffs(diff_results, params);
toc
